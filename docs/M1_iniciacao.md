...
A variável objetivo corresponde ao estado do crédito do cliente, sendo utilizada para identificar situações de incumprimento e representada pela variável *Creditability*, de natureza binária, em que 1 indica cumprimento das obrigações de crédito e 0 indica incumprimento.

A relevância deste problema prende-se com o aumento do crédito ao consumo e com a necessidade crescente de sistemas de apoio à decisão que permitam reduzir o risco associado à concessão de crédito. Além disso, este tipo de modelo pode tornar a análise de crédito mais consistente, apoiando a decisão humana com base nos dados e nos padrões encontrados no histórico dos clientes.
...
* **Divisão de Tarefas:** 
  ...
  * **Nota:** Todos os membros irão participar de forma equitativa em cada tarefa. 
* **Ferramentas de Colaboração:** *GitHub Projects* para Kanban, reuniões semanais via *Teams/Discord* e presencial quando necessário, *Google Docs*. 
...
* **Observações Relevantes:** O conjunto de dados apresenta uma estrutura adequada para a construção de modelos de classificação. No entanto, verifica-se a existência de desequilíbrio na variável objetivo (*Creditability*), com 700 observações (70%) correspondentes a cumprimento das obrigações de crédito e 300 (30%) a incumprimento. Este desequilíbrio poderá influenciar o desempenho do modelo, tornando necessária a utilização de métricas de avaliação apropriadas, nomeadamente o *Recall*, o *F1-Score* e a *AUC-ROC*.
...
Como o repositório de dados utilizado não apresentava informação suficientemente detalhada sobre o significado de cada variável, o grupo recorreu à descrição original do *dataset*, disponível no *UCI Machine Learning Repository*. Esta fonte permitiu compreender melhor a codificação das variáveis e realizar uma análise mais correta.

Fonte: https://archive.ics.uci.edu/dataset/144/statlog+german+credit+data

| Variável | Tipo de Variável | Valores | Descrição |
|--------------------|------------------|--------|----------|
...
Relativamente à variável objetivo (*Creditability*), observa-se um desequilíbrio entre as classes, com cerca de 70% dos registos correspondentes a cumprimento de obrigações de crédito e 30% a incumprimento.
